#!/usr/bin/env python3
"""
logging_setup.py — zentrale Logging-Konfiguration für alle RadioZapper-Module.

Ein Aufruf von setup() in radiozapper.main() konfiguriert den Root-Logger;
...
logging.getLogger("<name>") und schreiben dorthin. Zwei Ziele:
  ...
  - Konsole (stdout): das, was vorher per print() rausging — landet
    unverändert in `docker compose logs -f radiozapper`. Default INFO,
    mit --verbose DEBUG.
  - Logdatei (logs/radiozapper.log, rotierend): IMMER auf DEBUG, egal was
...
DEFAULT_LOG_FILE = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "logs", "radiozapper.log"
)
