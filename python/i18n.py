...
_BASE_STRINGS = {
    ...
    'idx_mark_german_title': 'Mark the currently recognized song as German-language (for future skipping)',
    'idx_mark_german_btn': '🇩🇪 German!',
    'idx_mark_german_done_btn': '✓ Marked as German',
    'idx_mark_german_saved': '🇩🇪 Marked as German.',
    'idx_news_break_skip_title': 'Play a different clip during the news break (break stays active)',
    'idx_news_break_skip_btn': '⏭ Different break clip',
    ...
    'idx_song_audd_quota': '⚠️ AudD quota exhausted',
    'idx_song_audd_network': '⚠️ AudD unreachable',
    'idx_song_audd_error': '⚠️ AudD error (code {code})',
    ...
    'idx_news_break_skip_switching': '📰 Picking a different clip …',
    ...
    'cfg_nb_adskip_enabled_label': 'skip the ad block after the break (experimental)',
    'cfg_nb_adskip_hint': 'In the last few seconds of the break MP3, already connects to the station in the background and listens for music — if it finds it in time, playback resumes directly in the music instead of a live ad block. Best-effort: no live radio stream can be fast-forwarded, so a longer ad block still plays live like today.',
    'cfg_nb_adskip_lead_label': 'lead time before break end (seconds)',
    'cfg_nb_speechgate_hint': 'Only starts the break once speech is currently detected on the live station in addition to the time window — prevents a purely time-based trigger while music is still playing. Without detected speech, the time window simply passes and the break is skipped for that occasion.',
    'cfg_nb_speechgate_enabled_label': 'only start the break when speech is detected (experimental)',
    'cfg_nb_speechgate_window_label': 'tolerance window around :00/:30 (minutes)',
    'cfg_nb_speechgate_streak_label': 'required consecutive speech windows',
    ...
    'cfg_update_check_heading': '🔄 Automatic update check',
    'cfg_update_check_hint': 'Checks every 24h via pure read access against GitHub whether the <code>main</code> branch is ahead of this installation. There is currently NO image registry deployment for the Docker installation — the only way to update is <code>git pull</code> in the repository directory, then <code>docker compose up -d --build radiosabbelnich</code>. No auto-update, no automatic installation — if available, only a hint appears at the top of this page and the player page.',
    'cfg_update_check_checkbox': 'Automatically check for updates',
    'update_banner_text': '🔄 Update available ({version}) — run git pull in the repository directory, then rebuild.',
    'update_banner_changelog_link': "What's new?",
    ...
    'cfg_stt_lang_delete_entry_only': 'Remove entry only',
    'cfg_stt_lang_delete_files_btn': '+ Delete files',
    'cfg_stt_lang_delete_files_confirm': "Really delete language '{code}' AND its downloaded model files from disk? This frees up space but cannot be undone — the model has to be downloaded again to use this language later. Categories assigned to it will fall back to German afterwards.",
    'cfg_stt_lang_download_btn': '⬇️ Download',
    'cfg_stt_lang_download_hint': 'Downloads a known Vosk model automatically, unpacks it and adds it above — no console access needed. The confidence threshold is set to a placeholder and should be calibrated afterwards using the wizard below.',
    'cfg_stt_lang_download_progress_bytes': '{done} MB of {total} MB ({pct}%)',
    'cfg_stt_lang_download_progress_unknown': '{done} MB downloaded …',
    'cfg_stt_lang_download_extracting': 'Extracting …',
    'cfg_stt_lang_download_installing': 'Installing …',
    'cfg_stt_lang_download_failed': 'Download failed: {error}',
    'cfg_stt_lang_download_done': "Language '{code}' downloaded and installed.",
    'cfg_stt_cat_lang_heading': '🏷 Category languages',
    'cfg_stt_cat_lang_hint': 'Sets which of the languages configured above is checked for stations of which category — only applies to stations without their own language set in the station list above. Categories without a selection default to German.',
    'cfg_stt_cat_lang_default': '(default: German)',
    'cfg_stt_cat_lang_saved': 'Category language saved.',
    'cfg_station_lang_label': 'STT language (overrides category)',
    'cfg_station_lang_default': '(default: category)',
    'cfg_station_lang_badge_title': 'STT language override for this station',
    ...
    'cfg_night_scan_heading': '🌙 Nightly station scan',
    'cfg_night_scan_hint': 'Automatically detects each station\'s spoken language via Whisper (no language-specific model needed, unlike Vosk above) and suggests it — NEVER applied automatically, every suggestion below must be confirmed. Most resource-friendly at night once the configured time window is reached; the "Scan now" button works anytime regardless.',
    'cfg_night_scan_hours_enabled_label': 'only active during certain hours (automatic run)',
    'cfg_night_scan_whisper_size_label': 'Whisper model size',
    'cfg_night_scan_timeout_label': 'Max. wait time per station (seconds)',
    'cfg_night_scan_min_speech_label': 'Minimum speech amount (seconds)',
    'cfg_night_scan_concurrency_label': 'Stations at once',
    'cfg_night_scan_saved': 'Nightly scan settings saved.',
    'cfg_night_scan_start_btn': '🌙 Scan now',
    'cfg_night_scan_stop_btn': 'Stop',
    'cfg_night_scan_starting': 'Starting …',
    'cfg_night_scan_progress': '{checked} of {total} checked ({current} …)',
    'cfg_night_scan_failed': 'Scan failed: {error}',
    'cfg_night_scan_done': 'Scan finished: {scanned} of {total} stations checked.',
    'cfg_night_scan_col_station': 'Station',
    'cfg_night_scan_col_result': 'Result',
    'cfg_night_scan_no_suggestions': 'No open suggestions.',
    'cfg_night_scan_result_detected': 'Detected: {language} ({pct}%)',
    'cfg_night_scan_result_music': 'Mostly music, no speech detected',
    'cfg_night_scan_result_unreachable': 'Not reachable',
    'cfg_night_scan_confirm_btn': 'Apply',
    'cfg_night_scan_dismiss_btn': 'Discard',
    'cfg_night_scan_confirmed': 'Language applied.',
    'cfg_night_scan_col_status': 'Status',
    'cfg_night_scan_status_active': 'active',
    'cfg_night_scan_status_disabled': 'disabled',
    'cfg_night_scan_coverage': '{activeScanned} of {activeTotal} active, {disabledScanned} of {disabledTotal} disabled stations scanned so far.',
    ...
    'cfg_songstats_heading': '🎵 Song recognition – statistics',
    'cfg_songstats_hint': 'Metrics from the existing song-recognition tables, for observing/calibrating -- purely informational, no settings here.',
    'cfg_songstats_disabled': 'Song recognition is disabled -- no statistics available.',
    'cfg_songstats_local_heading': 'Local fingerprinting (phase 1)',
    'cfg_songstats_entries': 'Entries in DB',
    'cfg_songstats_with_title': 'with title',
    'cfg_songstats_without_title': 'without title',
    'cfg_songstats_matchlog': 'Logged comparisons',
    'cfg_songstats_period': 'Collection period',
    'cfg_songstats_hitrate': 'Hit rate',
    'cfg_songstats_hitrate_hint': '(*) Tautological -- hit/miss is derived directly from the comparison against the current threshold, not an independent ground truth.',
    'cfg_songstats_threshold': 'Current threshold',
    'cfg_songstats_gap': 'Hit/miss separation',
    'cfg_songstats_gap_clean': 'Clean gap, e.g. {value} would separate all data seen so far',
    'cfg_songstats_gap_overlap': 'No clean gap -- miss max {miss}, hit min {hit}, overlapping',
    'cfg_songstats_gap_insufficient': 'Not enough data yet (need both hits and misses)',
    'cfg_songstats_percentiles_summary': 'Similarity percentiles & histogram',
    'cfg_songstats_hits': 'Hits',
    'cfg_songstats_misses': 'Misses',
    'cfg_songstats_stations_summary': 'Top stations',
    'cfg_songstats_songs_summary': 'Most-played recognized songs',
    'cfg_songstats_lang_german': '🇩🇪 German',
    'cfg_songstats_lang_not_german': '— not German',
    'cfg_songstats_lang_unknown': '❓ unclassified',
    'cfg_songstats_mark_german_btn': 'Mark German',
    'cfg_songstats_unmark_german_btn': 'Mark not German',
    'cfg_songstats_audd_heading': 'AudD cloud fallback (phase 2)',
    'cfg_songstats_audd_token': 'API token configured',
    'cfg_songstats_yes': 'yes',
    'cfg_songstats_no': 'no',
    'cfg_songstats_audd_laststatus': 'Last status',
    'cfg_songstats_audd_status_none': '– (no attempt yet)',
    'cfg_songstats_audd_status_ok': '✅ OK',
    'cfg_songstats_audd_status_quota': '⚠️ Quota exhausted',
    'cfg_songstats_audd_status_network': '⚠️ Unreachable',
    'cfg_songstats_audd_status_error': '⚠️ Error (code {code})',
    'cfg_songstats_audd_today': 'Requests today',
    'cfg_songstats_audd_week': 'Requests last 7 days',
    'cfg_songstats_audd_total': 'Requests total',
    'cfg_songstats_audd_since': 'since',
    'cfg_songstats_audd_outcomes': 'Outcomes (hit/no match/quota/error/network)',
    'cfg_songstats_audd_successrate': 'Success rate',
    'cfg_songstats_audd_cost': 'Estimated cost',
    'cfg_songstats_audd_cost_free': 'within free tier ({quota} requests)',
    'cfg_songstats_audd_cost_hint': 'Counted only since this statistics feature was introduced (see "requests total") -- does NOT reflect the actual AudD account balance if the quota was already (partially) used before. AudD provides neither a response field nor a lookup endpoint for that.',
    ...
    'cfg_update_check_saved': 'Update-check setting saved.',
