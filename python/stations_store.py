...
Gemeinsam genutzt von radiosabbelnich.py (Playback-Rotation) und webui.py
...
ankommen (radiosabbelnich.py pollt state.pop_reload_request() und liest bei
...
def bulk_add(entries: list, category: str = DEFAULT_CATEGORY, enabled: bool = True) -> list:
    ...
    lahmgelegt (siehe Watchdog in radiosabbelnich.py)."""
    if category not in CATEGORIES:
        category = DEFAULT_CATEGORY
    with _lock:
        stations = _read_raw()
